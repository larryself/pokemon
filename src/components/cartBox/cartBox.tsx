...
export const CartBox = () => {
	...
	const filterByTypes = (arr: PropsPokemon[]) => {
		...
		return arr.filter((item) => {
			const arr = item.types.filter((type) =>
				data.filter.type.includes(type.type.name)
			);
			return arr.length;
		});
	};
	const filterByName = (arr: PropsPokemon[]) => {
		...
		return data.filter.name
			? arr.filter(
					(pokemon) => pokemon.name.indexOf(data.filter.name.toLowerCase()) >= 0
			  )
			: arr;
	};
	const filterByAttack = (arr: PropsPokemon[]) => {
		...
		return arr.filter(
			(pokemon) =>
				pokemon.stats[1].base_stat >= attack[0] &&
				pokemon.stats[1].base_stat <= attack[1]
		);
	};
	const filterByExp = (arr: PropsPokemon[]) => {
		...
		return arr.filter(
			(pokemon) =>
				pokemon.base_experience >= exp[0] && pokemon.base_experience <= exp[1]
		);
	};
	const fiterPokemons = (arr: PropsPokemon[]) => {
		...
		const filteredByName = !data.filter.name
			? filteredByType
			: filterByName(filteredByType);
		const filteredByAttack = !data.filter.attack.length
			? filteredByName
			: filterByAttack(filteredByName);
		const filteredByExp = !data.filter.exp.length
			? filteredByAttack
			: filterByExp(filteredByAttack);
	...
	return (
		<>
			<Container>
				...
				{filteredPokemons
					.slice(0, data.currentPage * data.pokemonPerPage)
					.map((el: PropsPokemon, index, array) => {
