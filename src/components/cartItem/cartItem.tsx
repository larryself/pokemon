...
export const CartItem: FC<Props> = (props) => {
  ...
  return (
    <Container bgColor={GRADIENT[props.data.color]} onClick={()=> openCart(props.data)}>
      <Info>
        <Name>{props.data.name[0].toUpperCase() + props.data.name.slice(1)}</Name>
        <Stats>
          <StatBox>
            <StatValueBox>
        ...
        <Features>
          ...
          props.data.types.map((type: { type: { name: string } }) => (
            <Feature key={type.type.name} color={!type.type.name ? '' : featuresColors[type.type.name]  }>
      ...
      <ImgInner>
        {props.data.sprites &&
        <Image src={props.data.sprites.other['official-artwork'].front_default || ''} alt={props.data.name} width={168}
               height={156}/>}
      </ImgInner>
    </Container>
  );
