...
export const LegendPage = () => {
  ...
  const getStrongerPokemon = (pokemons: PokemonProps[]) => {
    return pokemons.filter((pokemon) => {
      return pokemon.stats[1].base_stat >= 100 && pokemon.stats[3].base_stat >= 100;
    });
  };

  const getWeaketPokemon = (pokemons: PokemonProps[]) => {
    return pokemons.filter((pokemon) => {
      return pokemon.stats[1].base_stat <= 30 && pokemon.stats[3].base_stat <= 30;
    });
  };
